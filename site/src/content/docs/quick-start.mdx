...
native VT library. Upstream Ghostty supports Zig 0.16, but the published Rust bindings still pin an
older revision. Betamax needs Zig 0.15.2 until a `libghostty-rs` release includes the update.
...
`PATH`. See [build troubleshooting](../reference/development/#build-troubleshooting) if Cargo finds
the wrong Zig version.
For SDK setup on Tahoe, see
[Building on macOS Tahoe](../reference/development/#building-on-macos-tahoe).
